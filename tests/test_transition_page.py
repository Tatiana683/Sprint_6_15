...
class TestTransitionPages:
    @allure.description('В кейсе проверяется переход на страницы при клике по элементу "Яндекс" логотипа')
    ...
    @allure.description('В кейсе проверяется переход на страницы при клике по элементу "Самокат" логотипа')
